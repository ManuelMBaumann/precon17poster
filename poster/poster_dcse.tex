...
\rput[tl](\leftcolspace,181mm){
  \posterroundedtabshadowbox{white}{\fbsdefault}{\leftcolwidth}{\large Preconditioning the elastic operator in 2D and 3D}{
    ...
    For the \textbf{3D operator}, we consider a splitting $\mathcal{P}=L+D+U$:
    \abovedisplayskip=0.8mm
    \belowdisplayskip=0.3mm
     \begin{align*}
 ...
 \end{align*}
    ...
    \item Additive {\color{matheonblue}coarse grid correction} yields grid-independent convergence ($H \ll h$).
%     \item
    \end{itemize}
